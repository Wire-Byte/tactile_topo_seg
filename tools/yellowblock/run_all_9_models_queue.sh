...
mkdir -p logs/yellowblock/mmseg logs/yellowblock/revise work_dirs/yellowblock/mmseg work_dirs/yellowblock/revise
...
run_with_guard() {
  ...
  while (( attempt <= MAX_RETRIES )); do
    ...
    set +e
    # Run in a dedicated process group so watchdog can kill all children reliably.
    # Avoid a pipe here: if we pipe to tee, $! may track tee instead of the real trainer.
...
run_mmseg_tpseg() {
  ...
  if [[ "$name" == "segformer_b2_skeleton_yellowblock_stage2_4090_bs4_stable" ]]; then
    ...
    # This model occasionally stalls with multi-worker loading in this environment.
...
run_external_tpseg() {
  ...
  local cfg="$2"
  local log_file="$3"
  local cmd="cd '$ROOT' && source /root/miniconda3/etc/profile.d/conda.sh && conda activate tpseg && export PYTHONPATH='$ROOT:$ROOT/third_party/mmsegmentation:\${PYTHONPATH:-}' && python -u tools/yellowblock/train_revise_external_yellowblock.py --config '$cfg'"
  run_with_guard "$name" tpseg "$cfg" "$log_file" "$cmd"
}
...
run_external_vmunet() {
  ...
  local cfg="$2"
  local log_file="$3"
  local cmd="cd '$ROOT' && source /root/miniconda3/etc/profile.d/conda.sh && conda activate vmunet && export PYTHONPATH='$ROOT:$ROOT/third_party/mmsegmentation:\${PYTHONPATH:-}' && python -u tools/yellowblock/train_revise_external_yellowblock.py --config '$cfg'"
  run_with_guard "$name" vmunet "$cfg" "$log_file" "$cmd"
}
...
declare -a MODEL_NAMES=(
  ...
  "bevanet_s_yellowblock_revise_v2_pretrain_smoke500"
  "emcad_yellowblock_revise_v2_pretrain_smoke500"
  "vmunet_yellowblock_revise_v2_pretrain_smoke500"
)

declare -a MODEL_RUNNERS=(
  "run_mmseg_tpseg"
  "run_mmseg_tpseg"
  "run_mmseg_tpseg"
  "run_mmseg_tpseg"
...
declare -a MODEL_CFGS=(
  ...
  "configs/yellowblock/bevanet_s_yellowblock_revise_v2_pretrain_smoke500.py"
  "configs/yellowblock/emcad_yellowblock_revise_v2_pretrain_smoke500.py"
  "configs/yellowblock/vmunet_yellowblock_revise_v2_pretrain_smoke500.py"
)

declare -a MODEL_LOGS=(
  "logs/yellowblock/mmseg/train_segformer_b2_yellowblock_smoke500.log"
  "logs/yellowblock/mmseg/train_segformer_b2_skeleton_yellowblock_stage2_4090_bs4_stable_smoke500.log"
  "logs/yellowblock/mmseg/train_segformer_b2_skeleton_cldice_v2_yellowblock_4090_bs4_smoke500.log"
  "logs/yellowblock/mmseg/train_pspnet_r50_yellowblock_main_table1_B_smoke500.log"
  "logs/yellowblock/mmseg/train_upernet_r50_yellowblock_main_table1_B_smoke500.log"
  "logs/yellowblock/mmseg/train_deeplabv3plus_r50_yellowblock_main_table1_B_smoke500.log"
  "logs/yellowblock/revise/train_bevanet_s_yellowblock_revise_v2_pretrain_smoke500.log"
  "logs/yellowblock/revise/train_emcad_yellowblock_revise_v2_pretrain_smoke500.log"
  "logs/yellowblock/revise/train_vmunet_yellowblock_revise_v2_pretrain_smoke500.log"
)
...
echo "[queue] Restarting smoke queue from model index ${START_INDEX}"
...
for ((i = START_INDEX - 1; i < ${#MODEL_NAMES[@]}; i++)); do
  ...
  "$runner" "$name" "$cfg" "$log_file"
done

echo "[queue] All requested models finished"
